...
const INIT_MESSAGES = [
  ...
  { id:4, from:"Sara Meziane",      avatar:"SM", role:"Student",       time:"Yesterday", date:"Yesterday",   subject:"Payment receipt",                 body:"Hi, I have sent my payment receipt for this month. Please confirm once you have received it. The reference number is TXN-20260415.",              unread:false, starred:true  },
...
const Icon = ({ name, size = 16 }) => {
  ...
  const icons = {
    home:     <svg viewBox="0 0 24 24" style={s} {...p}><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg>,
    profile:  <svg viewBox="0 0 24 24" style={s} {...p}><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg>,
    ...
    students: <svg viewBox="0 0 24 24" style={s} {...p}><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><path d="M23 21v-2a4 4 0 0 0-3-3.87"/><path d="M16 3.13a4 4 0 0 1 0 7.75"/></svg>,
    courses:  <svg viewBox="0 0 24 24" style={s} {...p}><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg>,
    ...
    trash:    <svg viewBox="0 0 24 24" style={s} {...p}><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/></svg>,
    ...
    more:     <svg viewBox="0 0 24 24" style={s} {...p}><circle cx="12" cy="5" r="1"/><circle cx="12" cy="12" r="1"/><circle cx="12" cy="19" r="1"/></svg>,
...
function StatCard({ icon, label, value, sub, color, warn }) {
  return (
    <div className={`tc-stat tc-stat--${color}`}>
      <div className="tc-stat-ic"><Icon name={icon} size={18} /></div>
      <div className="tc-stat-body">
        <span className="tc-stat-lbl">{label}</span>
        <strong className="tc-stat-val">{value}</strong>
        {sub && <span className={warn ? "tc-stat-sub--warn" : "tc-stat-sub"}>{sub}</span>}
      </div>
    </div>
  );
}
...
/* ══════════════════════════════════════════════════════════════
   HOME PAGE
══════════════════════════════════════════════════════════════ */
function HomePage({ students, messages, onNavigate }) {
  ...
  return (
    <div className="tc-page">
      ...
      <div className="tc-stats">
        <StatCard color="blue"  icon="students" label="Total Students" value={totalStudents} sub="+5 this month" />
        <StatCard color="green" icon="courses"  label="Courses" value={totalCourses} sub="6 languages" />
        <StatCard color="amber" icon="students" label="Absences Today" value={absentCount} sub={`${presentCount} present`} />
        <StatCard color="blue"  icon="mail"     label="Unread Messages" value={unreadCount} sub={`${unreadCount} unread`} />
      </div>
        ...
        <div className="tc-panel">
          <div className="tc-panel-header">
            <div>
              ...
              <span className="tc-panel-sub">Wednesday, April 29</span>
            </div>
              ...
              <svg width="90" height="90" viewBox="0 0 90 90">
                {(() => {
                  ...
                  return (<>
                    ...
                    <circle cx={cx} cy={cy} r={r} fill="none" stroke="#2D7A3A" strokeWidth={sw}
                      strokeDasharray={`${pPct * C} ${C}`}
                      style={{ transform: "rotate(-90deg)", transformOrigin: `${cx}px ${cy}px` }} />
                    <text x={cx} y={cy - 3} textAnchor="middle" fontSize="14" fontWeight="600" fill="#111827">{Math.round(pPct * 100)}%</text>
                    <text x={cx} y={cy + 12} textAnchor="middle" fontSize="8" fill="#9BA5B5">present</text>
                  </>);
                })()}
              </svg>
    ...
    <div className="tc-page">
      ...
      {saved && (
        <div className="tc-toast">
          <Icon name="check" size={15} />Profile saved successfully!
        </div>
      )}
      ...
      <div className="tc-profile-page-grid">
        ...
        <div className="tc-panel tc-profile-card">
          ...
          <div className="tc-profile-stats-row">
            <div className="tc-profile-stat"><strong>8</strong><span>Students</span></div>
            <div className="tc-profile-stat"><strong>6</strong><span>Courses</span></div>
        ...
        <div className="tc-panel tc-profile-fields">
          ...
          {editing && (
            <div className="tc-field-actions">
              ...
              <button className="tc-btn-primary" onClick={handleSave}><Icon name="check" size={13} />Save changes</button>
            </div>
          )}
        </div>
      </div>
      ...
      {/* Security */}
      <div className="tc-panel">
        <span className="tc-section-label" style={{ marginBottom: 14, display: "block" }}>Security</span>
        <div className="tc-security-row">
          ...
          <div className="tc-field-actions" style={{ marginTop: 4, gridColumn: "1/-1" }}>
            <button className="tc-btn-primary"><Icon name="lock" size={13} />Update password</button>
          </div>
        </div>
      </div>
    </div>
  ...
  return (
    <div className="tc-page">
      <div className="tc-page-header">
        ...
        <button className="tc-btn-primary"><Icon name="download" size={13} />Export</button>
      </div>

      <div className="tc-stats" style={{ gridTemplateColumns: "repeat(3,1fr)" }}>
        <StatCard color="blue"  icon="calendar" label="Classes/Week"    value={totalWeekClasses} sub="across 6 days" />
        <StatCard color="green" icon="students" label="Students reached" value={totalStudents}   sub="total enrolments" />
        <StatCard color="amber" icon="trend"    label="Teaching hours"  value={`${teachingHours}h`} sub="per week" />
      </div>
      ...
      <div className="tc-panel">
        <div className="tc-panel-header">
          <div>
            ...
            <span className="tc-panel-sub">Click a day to view its classes</span>
          </div>
        </div>
              ...
              <div key={i} className={`tc-tt-item tc-tt-item--${c.color}`}>
                ...
                <div className="tc-tt-body">
                  ...
                  <div className="tc-tt-meta">
                    <span><Icon name="book" size={11} />{c.room}</span>
                    <span><Icon name="students" size={11} />{c.students} students</span>
                  </div>
                </div>
...
function StudentsPage({ students, setStudents }) {
  ...
  const [newStudent, setNewStudent] = useState({ name: "", email: "", phone: "", pay: "", date: "", language: "", level: "", present: true });
  ...
  const addStudent = () => {
    ...
    setNewStudent({ name: "", email: "", phone: "", pay: "", date: "", language: "", level: "", present: true });
    setShowModal(false);
  ...
  return (
    <div className="tc-page">
      ...
      <div className="tc-stats" style={{ gridTemplateColumns: "repeat(3,1fr)" }}>
        <StatCard color="blue"  icon="students" label="Total Enrolled" value={students.length} sub="+5 this month" />
        <StatCard color="green" icon="check"    label="Present Today"  value={presentCount}    sub={`${Math.round(presentCount / students.length * 100)}% rate`} />
        <StatCard color="amber" icon="bell"     label="Absent Today"   value={absentCount}     sub="needs follow-up" />
      </div>

      <div className="tc-panel">
        <div className="tc-panel-header">
          <div>
            <span className="tc-panel-title">Student Records</span>
            <span className="tc-panel-sub">Manage attendance and payments</span>
          </div>
        </div>
        <div className="tc-toolbar">
          <div className="tc-toolbar-left">
            <button className="tc-btn-primary" onClick={() => setShowModal(true)}>
              <Icon name="plus" size={13} />Add student
            </button>
          </div>
                  ...
                  <tr key={s.id} className={selected === s.id ? "tc-row-selected" : ""} onClick={() => setSelected(selected === s.id ? null : s.id)}>
                    ...
                    <td>
                      <div className="tc-actions">
                        <button className="tc-action-btn" title="View details" onClick={e => { e.stopPropagation(); setSelected(selected === s.id ? null : s.id); }}>
                          <Icon name="eye" size={14} />
                        </button>
                        <button className="tc-action-btn" title="Delete" onClick={e => deleteStudent(s.id, e)}>
                          <Icon name="trash" size={14} />
                        </button>
                        <button className="tc-action-btn" title="More" onClick={e => e.stopPropagation()}>
          ...
          return (
            <div className="tc-student-detail">
              <div className="tc-student-detail-header">
                ...
                <span className={`tc-status ${s.present ? "tc-status--present" : "tc-status--absent"}`} style={{ marginLeft: "auto" }}>
                  <span className="tc-dot" />{s.present ? "Present" : "Absent"}
                </span>
              </div>
              ...
              <div className="tc-student-detail-actions">
                <button className="tc-btn-primary"><Icon name="send" size={13} />Send message</button>
      ...
      {toast && <div className="tc-toast"><Icon name="check" size={15} />{toast}</div>}
        ...
        <div className="tc-modal-overlay" onClick={() => setShowModal(false)}>
          <div className="tc-modal" onClick={e => e.stopPropagation()}>
            ...
            <div className="tc-modal-body">
              <div className="tc-form-grid2">
                <div className="tc-field"><label>Full Name *</label><input placeholder="e.g. Mohammed Kaci" value={newStudent.name} onChange={e => setNewStudent({ ...newStudent, name: e.target.value })} /></div>
              ...
              <div className="tc-form-grid2">
                <div className="tc-field"><label>Phone</label><input type="tel" placeholder="+213 5xx xxx xxx" value={newStudent.phone} onChange={e => setNewStudent({ ...newStudent, phone: e.target.value })} /></div>
                <div className="tc-field"><label>Payment</label><input placeholder="200DA" value={newStudent.pay} onChange={e => setNewStudent({ ...newStudent, pay: e.target.value })} /></div>
              </div>
              <div className="tc-form-grid2">
                <div className="tc-field">
                  <label>Language</label>
                  <select className="tc-select" style={{ width: "100%" }} value={newStudent.language} onChange={e => setNewStudent({ ...newStudent, language: e.target.value })}>
                    <option value="">Select…</option>
                    {["English", "French", "Spanish", "German", "Mandarin", "Arabic"].map(l => <option key={l}>{l}</option>)}
                  </select>
                </div>
                <div className="tc-field">
                  <label>Level</label>
                  <select className="tc-select" style={{ width: "100%" }} value={newStudent.level} onChange={e => setNewStudent({ ...newStudent, level: e.target.value })}>
                    <option value="">Select…</option>
                    {["A1", "A2", "B1", "B2", "C1", "C2"].map(l => <option key={l}>{l}</option>)}
                  </select>
                </div>
              </div>
              <div className="tc-field"><label>Enrolment Date</label><input type="text" placeholder="29 Apr 2026" value={newStudent.date} onChange={e => setNewStudent({ ...newStudent, date: e.target.value })} /></div>
            </div>
            <div className="tc-modal-footer">
              <button className="tc-btn-ghost" onClick={() => setShowModal(false)}>Cancel</button>
              <button className="tc-btn-primary" onClick={addStudent}><Icon name="plus" size={13} />Add Student</button>
            </div>
          </div>
        </div>
      )}
...
function CoursesPage({ courses, setCourses }) {
  ...
  const [form, setForm] = useState({ name: "", sessions: "", room: "", color: "blue", progress: 0 });
  const [toast, setToast] = useState(null);
  ...
  const addCourse = () => {
    if (!form.name) return;
    setCourses(prev => [...prev, { ...form, id: Date.now(), teacher: "Mohammed Bouchemot", students: 0, progress: Number(form.progress) }]);
    setForm({ name: "", sessions: "", room: "", color: "blue", progress: 0 });
    setShowAdd(false);
    ...
    <div className="tc-page">
      <div className="tc-page-header">
        ...
        <div style={{ display: "flex", gap: 10 }}>
          ...
          <button className="tc-btn-primary" onClick={() => setShowAdd(true)}><Icon name="plus" size={13} />Add course</button>
        </div>
      </div>
        ...
        {filtered.map(c => (
          <div key={c.id} className={`tc-course-card tc-course-card--${c.color}`}>
            <div className="tc-course-card-header">
              <div className={`tc-course-ic tc-course-ic--${c.color}`}><Icon name="courses" size={20} /></div>
              <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                <span className={`tc-course-badge tc-course-badge--${c.color}`}>{c.name.split(" ").pop()}</span>
                <button
                  style={{ background: "none", border: "none", cursor: "pointer", color: "var(--tc-text3)", padding: 4, borderRadius: 6 }}
            ...
            <div className="tc-course-meta">
              <div className="tc-course-meta-row"><Icon name="students" size={12} />{c.students} students</div>
              <div className="tc-course-meta-row"><Icon name="calendar" size={12} />{c.sessions}</div>
              <div className="tc-course-meta-row"><Icon name="book" size={12} />{c.room}</div>
            </div>
            <div className="tc-course-progress-wrap">
              <div className="tc-course-progress-header"><span>Completion</span><span>{c.progress}%</span></div>
      ...
      {showAdd && (
        <div className="tc-modal-overlay" onClick={() => setShowAdd(false)}>
          <div className="tc-modal" onClick={e => e.stopPropagation()}>
            <div className="tc-modal-header"><h3>Add New Course</h3><button className="tc-modal-close" onClick={() => setShowAdd(false)}>✕</button></div>
            <div className="tc-modal-body">
              <div className="tc-field"><label>Course Name *</label><input placeholder="e.g. English B2" value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} /></div>
              <div className="tc-form-grid2">
                <div className="tc-field"><label>Sessions</label><input placeholder="Mon, Wed, Fri" value={form.sessions} onChange={e => setForm({ ...form, sessions: e.target.value })} /></div>
                <div className="tc-field"><label>Room</label><input placeholder="A101" value={form.room} onChange={e => setForm({ ...form, room: e.target.value })} /></div>
              </div>
              <div className="tc-form-grid2">
                <div className="tc-field">
                  <label>Color</label>
                  <select className="tc-select" style={{ width: "100%" }} value={form.color} onChange={e => setForm({ ...form, color: e.target.value })}>
                    <option value="blue">Blue</option><option value="green">Green</option>
                    <option value="amber">Amber</option><option value="red">Red</option>
                  </select>
                </div>
                <div className="tc-field"><label>Progress (%)</label><input type="number" min="0" max="100" placeholder="0" value={form.progress} onChange={e => setForm({ ...form, progress: e.target.value })} /></div>
              </div>
            </div>
            <div className="tc-modal-footer">
              <button className="tc-btn-ghost" onClick={() => setShowAdd(false)}>Cancel</button>
              <button className="tc-btn-primary" onClick={addCourse}><Icon name="plus" size={13} />Add Course</button>
            </div>
          </div>
        </div>
      )}
    ...
    <div className="tc-page">
              ...
              : filtered.map(m => (
                <div key={m.id} className={`tc-msg-item${selected?.id === m.id ? " tc-msg-item--active" : ""}${m.unread ? " tc-msg-item--unread" : ""}`} onClick={() => openMsg(m)}>
                  ...
                  <div className="tc-msg-item-actions">
                    ...
                    <button className="tc-msg-del" onClick={e => deleteMsg(m.id, e)}><Icon name="trash" size={12} /></button>
                  </div>
                </div>
              ))
            }
          ...
          {!selected
            ...
            : <>
                <div className="tc-msg-view-header">
                  ...
                  <button className="tc-msg-del" style={{ marginLeft: 6 }} onClick={e => deleteMsg(selected.id, e)}><Icon name="trash" size={14} /></button>
                </div>
                ...
                <div className="tc-msg-reply">
                  ...
                  {sent && (
                    <div className="tc-toast tc-toast--inline">
                      <Icon name="check" size={13} />Reply sent!
                    </div>
                  )}
                  <div className="tc-msg-reply-footer">
                    <button className="tc-btn-ghost" onClick={() => setReply("")}>Clear</button>
                    <button className="tc-btn-primary" onClick={sendReply}><Icon name="send" size={13} />Send reply</button>
                  </div>
                </div>
              </>
          }
      ...
      {compose && (
        <div className="tc-modal-overlay" onClick={() => setCompose(false)}>
          <div className="tc-modal" onClick={e => e.stopPropagation()}>
            <div className="tc-modal-header"><h3>New Message</h3><button className="tc-modal-close" onClick={() => setCompose(false)}>✕</button></div>
            <div className="tc-modal-body">
              <div className="tc-field"><label>To</label><input placeholder="Recipient name or email" value={newMsg.to} onChange={e => setNewMsg({ ...newMsg, to: e.target.value })} /></div>
              <div className="tc-field"><label>Subject</label><input placeholder="Message subject" value={newMsg.subject} onChange={e => setNewMsg({ ...newMsg, subject: e.target.value })} /></div>
              <div className="tc-field"><label>Message</label><textarea rows={5} placeholder="Write your message…" value={newMsg.body} style={{ padding: "9px 12px", border: "var(--tc-border2)", borderRadius: "var(--tc-r)", background: "var(--tc-bg)", fontSize: 13, fontFamily: "var(--font-body)", resize: "vertical" }} onChange={e => setNewMsg({ ...newMsg, body: e.target.value })} /></div>
            </div>
            <div className="tc-modal-footer">
              <button className="tc-btn-ghost" onClick={() => setCompose(false)}>Cancel</button>
              <button className="tc-btn-primary" onClick={sendCompose}><Icon name="send" size={13} />Send</button>
            </div>
          </div>
        </div>
      )}
...
export default function TeacherDashboard() {
  ...
  // Shared state — passed down to pages so they can mutate
  ...
  const renderPage = () => {
    switch (activeNav) {
      case "Home":      return <HomePage students={students} messages={messages} onNavigate={setActiveNav} />;
      case "Profile":   return <ProfilePage />;
      case "Timetable": return <TimetablePage />;
      case "Students":  return <StudentsPage students={students} setStudents={setStudents} />;
      case "Courses":   return <CoursesPage courses={courses} setCourses={setCourses} />;
      case "Messages":  return <MessagesPage messages={messages} setMessages={setMessages} />;
      default:          return <HomePage students={students} messages={messages} onNavigate={setActiveNav} />;
    }
  };
    ...
    <div className={`tc-layout${collapsed ? " tc-collapsed" : ""}`}>
      ...
      <aside className="tc-sidebar">
        <div className="tc-logo">
          <div className="tc-logo-icon"><Icon name="book" size={16} /></div>
        ...
        {!collapsed && (
          <div className="tc-profile">
            ...
            <div className="tc-profile-info">
              <strong>Mohammed Bouchemot</strong>
              <span>Teacher</span>
            </div>
          </div>
        )}
        ...
        <nav className="tc-nav">
          {["main", "manage"].map(section => (
            <React.Fragment key={section}>
              {!collapsed && (
                <span className="tc-nav-label" style={section === "manage" ? { marginTop: 10 } : {}}>
              ...
              {navItems.filter(n => n.section === section).map(n => (
                <button key={n.label} className={`tc-nav-item${activeNav === n.label ? " tc-nav-active" : ""}`} onClick={() => setActiveNav(n.label)}>
                  <span className="tc-nav-icon"><Icon name={n.icon} size={16} /></span>
                  {!collapsed && (
                  ...
                  {collapsed && n.label === "Messages" && unreadCount > 0 && <span className="tc-notif-dot-collapsed" />}
                </button>
              ))}
            </React.Fragment>
          ))}
        ...
        <div className="tc-sidebar-bottom">
          <button className="tc-nav-item tc-nav-danger" onClick={() => {
            ...
            <span className="tc-nav-icon"><Icon name="logout" size={16} /></span>
      ...
      <div className="tc-main">
        <header className="tc-header">
          <div className="tc-header-left">
            ...
            <span className="tc-header-sub">April 29, 2026 · Spring Semester</span>
          </div>
          ...
          <div className="tc-header-right">
            <button className="tc-icon-btn" title="Messages" onClick={() => setActiveNav("Messages")} style={{ position: "relative" }}>
              ...
              {unreadCount > 0 && <span className="tc-badge-count">{unreadCount}</span>}
            </button>
