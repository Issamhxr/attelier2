// controllers/secretaireController.js
...
exports.getDashboardStats = async (req, res) => {
  try {
    const [totalStudents, pendingCount, sections, paymentsAgg, absencesCount] =
      await Promise.all([
...
exports.getPendingStudents = async (req, res) => {
  try {
    ...
    return res.json({ success: true, pending: formatted });
  } catch (err) {
    return res.status(500).json({ success: false, message: err.message });
  }
};
...
exports.registerStudent = async (req, res) => {
  try {
    ...
    const user = await User.create({
      ...
      password:  password  || "etudiant123",
      role:      "etudiant",
      telephone: telephone || "",
      language:  language  || "",
      level:     level     || "",
      section:   section   || "À assigner",
      actif:     false,
    });
...
exports.acceptStudent = async (req, res) => {
  try {
    ...
    const user = await User.findOneAndUpdate(
      { _id: req.params.id, role: "etudiant" },
      updates,
      { new: true }
    );
    ...
    if (section) {
      await Section.findOneAndUpdate(
        { name: section },
        { $addToSet: { students: user._id }, $inc: { studentsCount: 1 } }
      );
    }
...
/* ══════════════════════════════════════════════════
   INSCRIPTIONS — Rejeter
══════════════════════════════════════════════════ */
exports.rejectStudent = async (req, res) => {
  try {
    const user = await User.findOneAndDelete({ _id: req.params.id, role: "etudiant", actif: false });
    if (!user) {
      return res.status(404).json({ success: false, message: "Étudiant introuvable ou déjà actif." });
    }
    return res.json({ success: true, message: "Inscription rejetée et supprimée." });
  } catch (err) {
    return res.status(500).json({ success: false, message: err.message });
  }
};
...
exports.createSection = async (req, res) => {
  try {
    const { name, language, level, teacher, teacherId, time, room, capacity, flag } = req.body;
...
exports.getTimetable = async (req, res) => {
  try {
    ...
    const entries = await Timetable.find(filter)
      .populate("sectionId", "name language level")
      .sort({ startTime: 1 })
      .lean();
    ...
    const formatted = entries.map(e => ({
      ...
      room:      e.room,
      section:   e.sectionId?.name || e.section,
      subject:   e.sectionId?.language || e.subject,
      teacher:   e.teacher,
      dayOfWeek: e.dayOfWeek,
    }));

    return res.json({ success: true, timetable: formatted });
  } catch (err) {
    return res.status(500).json({ success: false, message: err.message });
  }
};
...
/* ══════════════════════════════════════════════════
   ABSENCES — Liste
   ⚠️ Le modèle Absence utilise `student` (ObjectId ref User)
══════════════════════════════════════════════════ */
exports.getAbsences = async (req, res) => {
  try {
    ...
    const absences = await Absence.find(filter)
      .populate("student", "nom prenom section")
      .sort({ date: -1 })
      .lean();

    const formatted = absences.map(a => ({
      id:        a._id,
      student:   `${a.student?.prenom || ""} ${a.student?.nom || ""}`.trim() || "—",
    ...
    // Filtre section côté JS (pas dans le modèle Absence)
    const result = section
      ? formatted.filter(a => a.section === section)
      : formatted;
...
exports.createAbsence = async (req, res) => {
  try {
    ...
    // Supporte aussi l'ancien format (student=nom, studentId=objectId)
    const resolvedStudentId = studentId || req.body.student;

    if (!resolvedStudentId || !date) {
      return res.status(400).json({ success: false, message: "Étudiant et date obligatoires." });
    }

    // Vérifier si c'est un ObjectId valide
    const mongoose = require("mongoose");
    if (!mongoose.Types.ObjectId.isValid(resolvedStudentId)) {
    ...
    await User.findByIdAndUpdate(resolvedStudentId, { $inc: { absences: 1 } });
...
exports.getPayments = async (req, res) => {
  try {
    ...
    const formatted = payments.map(p => ({
      id:      p._id,
      name:    `${p.student?.prenom || ""} ${p.student?.nom || ""}`.trim() || p.studentName || "—",
      amount:  p.amount,
      date:    new Date(p.createdAt).toLocaleDateString("fr-DZ"),
...
exports.createPayment = async (req, res) => {
  try {
    ...
    if (!amount || !due) {
      return res.status(400).json({ success: false, message: "Montant et date obligatoires." });
    }

    // Récupérer la langue/niveau de l'étudiant si possible
...
/* ══════════════════════════════════════════════════
   NOTIFICATIONS — Liste
   ⚠️ Le modèle Notification utilise `targets` (array de strings),
      pas `recipient` ou `recipientRole`
══════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════
   ...
   ⚠️ Le modèle Message utilise `sender`/`recipient`
══════════════════════════════════════════════════ */
...
exports.sendMessage = async (req, res) => {
  try {
    ...
    if (!destId || !subject || !body) {
      return res.status(400).json({ success: false, message: "Destinataire, sujet et corps obligatoires." });
    }
