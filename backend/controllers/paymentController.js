...
exports.getPayments = async (req, res) => {
  try {
    ...
    // Mise à jour automatique des overdue
  ...
  try {
    ...
    if (!amount || !due) {
      return res.status(400).json({
        success: false,
        message: "Champs obligatoires : amount et due.",
      });
    }
    ...
    const payment = await Payment.create({
      ...
      amount:   parsedAmount,
      paid:     parsedAmount, // paiement comptant = soldé
      due:      new Date(due),
...
exports.registerPayment = async (req, res) => {
  try {
    ...
    if (payment.status === "paid") {
      return res.status(400).json({ success: false, message: "Cette facture est déjà soldée." });
    }

    // Si montant non précisé → solde le reste
    const versement = amount
      ? parseFloat(amount)
    ...
    await payment.save(); // pre('save') recalcule le statut
