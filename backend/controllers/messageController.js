...
exports.getMessage = async (req, res) => {
  try {
    ...
    if (recipientId === req.user.id && !message.read) {
      message.read = true;
      await message.save();
    }
...
exports.sendMessage = async (req, res) => {
  try {
    // Support both { to, recipientId } field names
    ...
    if (!recipientId || !subject || !body) {
      return res.status(400).json({
        success: false,
        message: "Destinataire, sujet et corps requis.",
      });
    }
...
exports.replyMessage = async (req, res) => {
  try {
    ...
    if (!body) {
      return res.status(400).json({ success: false, message: "Le corps de la réponse est requis." });
    }

    const original = await Message.findById(req.params.id);
    if (!original) {
      return res.status(404).json({ success: false, message: "Message original introuvable." });
    }
...
exports.deleteMessage = async (req, res) => {
  try {
    ...
    const isOwner = senderId === req.user.id || recipientId === req.user.id;
